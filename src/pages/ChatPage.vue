...
const handleChatMessage = (msg: any) => {
  ...
  if (msg.type === 'onlineUsers') {
    console.log('收到在线用户消息:', msg)
    // 查找目标用户
    const targetUserObj = msg.onlineUsers?.find(
      (user: any) => String(user.id) === String(targetUser.value?.id)
    )
    console.log('找到的目标用户:', targetUserObj)
    
    // 根据online字段判断在线状态
    const previousStatus = isOnline.value
    isOnline.value = targetUserObj?.online === true
    
    console.log('目标用户在线状态变化:', previousStatus, '->', isOnline.value, '用户角色:', targetUserObj?.online)
  }
  // 处理历史消息和更多历史消息
...
const handleAvatarClick = () => {
  console.log('头像点击:', targetUser.value)
  router.push({
