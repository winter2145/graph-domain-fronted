...
const getFollowAndFansCount = async () => {
  ...
  console.log('Fetching follow and fans count for userId:', userId)
  const res = await getFollowAndFansCountUsingPost({ id: String(userId) })

    if (res.data.code === 0) {
      console.log('Follow and Fans Count Response:', res)
...
const startPrivateChat = async () => {
  try {
    ...
    if (res.data?.code === 0 && res.data.data) {
      ...
      message.error(res.data?.message ||'创建私聊失败')
    }
  } catch (error) {
