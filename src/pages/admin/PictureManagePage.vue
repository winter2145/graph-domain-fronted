  ...
  <div id="pictureManagePage">
    ...
    <template v-if="device === DEVICE_TYPE_ENUM.PC">
      <div class="pc-container">
        <!-- 搜索表单和操作按钮 -->
        <a-form layout="inline" :model="searchParams" @finish="doSearch" class="search-form-flex">
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-if="column.dataIndex === 'url'">
                <div class="table-img-cell">
                  <img :src="record.url" class="table-img" />
                </div>
              </template>
              ...
              <template v-if="column.dataIndex === 'reviewMessage'">
                <div>
                  审核状态：<span :class="getStatusColorClass(Number(record.reviewStatus))">{{
                    PIC_REVIEW_STATUS_MAP[Number(record.reviewStatus)]
                  }}</span>
                </div>
                ...
                <van-card class="picture-card">
                  ...
                  <template #tags>
                    <div class="tag-container">
                      <van-tag
                        :type="getReviewStatusType(Number(picture.reviewStatus))"
                        round
                        size="small"
                        style="margin-right: 4px"
                      >
                        {{ PIC_REVIEW_STATUS_MAP[Number(picture.reviewStatus)] }}
                      </van-tag>
...
const handleRejectReasonChange = (value: keyof typeof REJECT_REASON_MAP) => {
  if (value !== 'other') {
...
const doSearch = () => {
  searchParams.current = 1
    // 转换输入为数组
    searchParams.tags = tagsInput.value
...
.search-form-flex .ant-form-item {
  margin-bottom: 0;
}

/* 表格图片单元格内容垂直居中，图片最大高度限制 */
:deep(.ant-table-cell) {
...
.table-img:hover, .table-img-cell img:hover,
:deep(.ant-table-cell):hover .table-img,
:deep(.ant-table-cell):hover img {
...
@media screen and (max-width: 768px) {
  ...
  .card-title, .card-desc, .desc-item, .card-footer {
    word-break: break-all;
    white-space: normal;
    overflow: hidden;
  }
