...
const columns = [
  {
    title: '序号',
    key: 'index',
    customRender: ({ index }: { index: number }) => index + 1 + (searchParams.current - 1) * searchParams.pageSize,
  },
  {
    title: '标签ID',
    dataIndex: 'id',
...
const searchTagList = async () => {
  try {
    // 只传tagName，避免类型报错
    const res = await searchTagUsingGet({
      tagName: searchParams.tagName as string,
    })
    if (res.data.code === 0 && res.data.data) {
