    ...
    <template v-if="comments && comments.length > 0">
      <div v-for="comment in comments" :key="comment.id || comment.commentId" class="comment-item">
        ...
        <div class="comment-header">
          <a-avatar :src="comment.commentUser?.userAvatar || getDefaultAvatar(comment.commentUser?.userName)" 
          :size="40" />
          ...
          <template v-if="comment.targetType === 1">
            ...
            <div class="target-preview picture-preview">
              <div class="target-type">
                <PictureOutlined />
                <span :style="Number(comment.parentCommentId) > 0 ? 'color: #ff4d4f; font-weight: 500;' : ''">
                  {{
                    isReceived
                      ? (Number(comment.parentCommentId) > 0 ? '回复了你的评论' : '评论了你的图片')
                      : (Number(comment.parentCommentId) > 0 ? '你回复的评论' : '你评论的图片')
                  }}
                </span>
              </div>
              <img
                :src="comment.picture?.thumbnailUrl || '@/assets/image-placeholder.png'"
