  ...
  <div class="unread-like-list">
    <div v-for="like in likes" :key="like.id" class="like-item">
      ...
      <div class="like-header">
        <a-avatar :src="like.user.userAvatar || getDefaultAvatar(like.user?.userName)" 
        :size="40" />
        <div class="like-info">
      ...
      <div class="like-target" @click="handleTargetClick(like)">
        ...
        <template v-else-if="like.targetType === 4">
          <!-- 评论类型 -->
          <div class="target-preview comment-preview">
            <div class="target-type">
              <MessageOutlined />
              <span>{{ isReceived ? '赞了你的评论' : '你赞过的评论' }}</span>
            </div>
            <div class="target-info">
              <div class="target-content">{{ like.target.content }}</div>
              <div class="comment-meta">
                <span class="comment-user">{{ like.target?.commentUser?.userName || '未知用户' }}</span>
                <span class="comment-time">{{ formatTime(like.target?.createTime) }}</span>
              </div>
            </div>
          </div>
        </template>
...
import { LikeOutlined, PictureOutlined, FileTextOutlined, MessageOutlined } from '@ant-design/icons-vue'
...
const handleTargetClick = (like: any) => {
  if (like.targetType === 1) {
    ...
    // 跳转到评论所属的内容详情（根据 targetType 判断）
    if (like.target.targetType === 1) {
      // 评论属于图片
      router.push(`/picture/${like.target.targetId}`)
    } else if (like.target.targetType === 2) {
      // 评论属于帖子
      router.push(`/post/${like.target.targetId}`)
    }
  }
}
